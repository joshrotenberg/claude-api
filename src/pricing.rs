...
impl Default for PricingTable {
    fn default() -> Self {
        // Bundled rates -- best-effort. Override via PricingTable::custom for
        // billing-critical workloads.
        let mut rates: HashMap<ModelId, ModelPricing> = HashMap::new();
        rates.insert(
            ModelId::OPUS_4_7,
            ModelPricing::from_input_output(15.00, 75.00, 0.01),
        );
        rates.insert(
            ModelId::SONNET_4_6,
            ModelPricing::from_input_output(3.00, 15.00, 0.01),
        );
        rates.insert(
            ModelId::HAIKU_4_5,
            ModelPricing::from_input_output(1.00, 5.00, 0.01),
        );
        Self { rates }
    }
}
